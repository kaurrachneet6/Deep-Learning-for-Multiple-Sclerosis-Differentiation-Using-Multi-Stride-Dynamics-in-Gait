...
class GRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_size1, num_layers1, hidden_size2, num_layers2, num_classes, dropout, bidirectional1, bidirectional2, pre_out, single_layer, linear_size, device, use_layernorm):
        ...
        #Try splitting GRU into multiple with different hidden sizes 
        ...
        #Does num_layers actually make multiple GRU layers?
        #self.GRU = nn.GRU(pre_out, hidden_size1, num_layers1, batch_first=True, bidirectional = bidirectional1) #uncomment for first linear layer

        ##Use these two for two seperate GRU layers
        self.GRU = nn.GRU(self.input_size, self.hidden_size1, self.num_layers1, batch_first=True, bidirectional = self.bidirectional1) #uncomment for first GRU layer

        #self.GRU = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, bidirectional = bidirectional) 
        self.GRU2 = nn.GRU(self.hidden_size1*self.bi_mult1, self.hidden_size2, self.num_layers2, batch_first=True, bidirectional = self.bidirectional2)
    ...
    def forward(self, x):
        ...
        #linear layer takes features in and then output as different dimention, change GRU size to match
        ...
        #, _ = self.GRU(out, (h0, c0))  # out: tensor of shape (batch_size, seq_length, hidden_size) #uncomment for first linear layer
        out, _ = self.GRU(x, h0)    #uncomment for first GRU layer
        #print("GRU1 out: ", out.shape)
        if (not self.single_GRU):
            out, _ = self.GRU2(out, h02)
        #print("GRU2 out: ", out.shape)
        ...
        #check: is GRU output 10x30x2? if so it is all timesteps output, we only want the last one out[:,:,-1]? shape should be 30x2 for last one
